...
def BytesToTime(b):
    ...
    mftime = struct.unpack('<Q', b.encode('latin-1'))[0]
    if mftime == 0x7fffffffffffffff:
        return None
    else:
        microseconds = struct.unpack('<Q', b.encode('latin-1'))[0] / 10.0
        nanoseconds = (struct.unpack('<Q', b.encode('latin-1'))
                       [0] % 10000000) * 100
...
def TimeToBytes(time):
    if time == None:
        # 0x7fffffffffffffff LE
        return '\xff\xff\xff\xff\xff\xff\xff\x7f'.encode('latin-1')
    else:
...
def AlignedString(s, alignment=4):
    try:
        return (s + '\x00' * ((alignment - (len(s) % alignment)) % alignment)).encode('latin-1')
    except:
        return (s.decode('latin-1') + '\x00' * ((alignment - (len(s) % alignment)) % alignment)).encode('latin-1')
    ...
    def __new__(cls, *args, **kwargs):
        if len(args) > 0 and type(args[0]) == datetime.datetime:
            ...
            if 'nanosecond' in kwargs:
                ...
                dt = datetime.datetime.__new__(
                    cls, dt.year, dt.month, dt.day, dt.hour, dt.minute, dt.second, dt.microsecond, dt.tzinfo, nanoseconds=0)
...
class PacInfoStructure(object, metaclass=ABCMeta):
    ...
    def __init__(self, pac, index=0):
        if input != None:
            ...
            self.Type, self.BufferSize, self.Offset = struct.unpack(
                '<III', pac[offset:offset+12].encode('latin-1'))
            self.Data = pac[self.Offset:self.Offset+self.BufferSize]

    def __str__(self):
        return '%s (%i) Size: %i  Offset: %i\nData: %s' % (self.PrettyName, self.Type, self.BufferSize, self.Offset, self.Data.encode('latin-1'))
...
class PacLoginInfo(PacInfoStructure):
    ...
    class Referent(object):
        ...
        def __init__(self, data):
            self.Length = struct.unpack('<H', data[0:2].encode('latin-1'))[0]
            self.Size = struct.unpack('<H', data[2:4].encode('latin-1'))[0]
            self.ReferentID = struct.unpack(
                '<L', data[4:8].encode('latin-1'))[0]
    ...
    def __init__(self, pac, index=0):
        ...
        # MessageHeader
        self.Version, self.ByteOrder, self.HeaderLen = struct.unpack(
            '<BBH', self.Data[0:4].encode('latin-1'))
        self.FillBytes = self.Data[4:8]
        self.InitialLength = struct.unpack(
            '<Q', self.Data[8:16].encode('latin-1'))[0]
        ...
        # print dirdriveref
        self.Logoncount, self.BadpPasswordCount, self.UserRid, self.GroupRid, numrids = struct.unpack(
            '<HHLLL', self.Data[116:132].encode('latin-1'))  # [0]
        ...
        self.AccountName = self.Data[offset:offset +
                                     accountnameref.Length]  # .decode('utf-8')
        # print '%i %s' % (offset, self.AccountName)
        offset += accountnameref.Length + accountnameref.PaddLen

        offset += 4 * 3  # max count, offset, actual count
        self.DisplayName = self.Data[offset:offset +
                                     fullnameref.Length]  # .decode('utf-8')
        ...
        maxridcount = struct.unpack(
            '<L', self.Data[offset:offset+4].encode('latin-1'))[0]
        # print 'maxrid', maxridcount
        offset += 4
        grouprids = []
        for i in range(numrids):
            # print self.Data[offset:offset+12].encode('hex')
            grouprids.append(struct.unpack(
                '<L', self.Data[offset:offset+4].encode('latin-1'))[0])
            offset += 8
    ...
    def encode(self):
        header = struct.pack('<BBH', self.Version, self.ByteOrder, self.HeaderLen).decode('latin-1') + \
            self.FillBytes
        ...
        e = struct.pack('<L', 0x00020000) + \
            ...
            struct.pack('<HHL', len(self.HomeDir),     len(self.HomeDir),     0x00020014) + \
            struct.pack('<HHL', len(self.DirDrive),    len(self.DirDrive),    0x00020018) + \
            struct.pack('<HHLLL', self.Logoncount, self.BadpPasswordCount,
                        self.UserRid, self.GroupRid, len(self.Groups))

        # TODO: Decode and re-encode this
        e += self.extrajunk.encode('latin-1')
...
class PacClientInfo(PacInfoStructure):
    ...
    def __init__(self, pac, index=0):
        ...
        namelen = struct.unpack('<H', self.Data[8:10].encode('latin-1'))[0]
...
class PacUpnDnsInfo(PacInfoStructure):
    ...
    def __init__(self, pac, index=0):
        ...
        upnlen = struct.unpack('<H', self.Data[:2].encode('latin-1'))[0]
        upnoffset = struct.unpack('<H', self.Data[2:4].encode('latin-1'))[0]
        dnslen = struct.unpack('<H', self.Data[4:6].encode('latin-1'))[0]
        dnsoffset = struct.unpack('<H', self.Data[6:8].encode('latin-1'))[0]
        self.Flags = self.Data[8:12]  # screw decoding this

        # print upnlen, upnoffset, dnslen, dnsoffset

        self.UPNName = self.Data[upnoffset:upnoffset+upnlen].encode('latin-1')
        self.DNSName = self.Data[dnsoffset:dnsoffset+dnslen].encode('latin-1')

    def __str__(self):
        return '%s (%i) Flags: %s  UPN Name: %s  DNS Name: %s' % (self.PrettyName, self.Type, '%0.8X' % struct.unpack('<L', self.Flags.encode('latin-1'))[0], self.UPNName, self.DNSName)
    ...
    def encode(self):
        ...
        try:
            return struct.pack('<HHHH', \
                ...
                self.Flags.encode('latin-1') + \
                b'\x00\x00\x00\x00' + \
                str(AlignedString(self.UPNName)).encode('latin-1') + \
                b'\x00\x00\x00\x00' + \
                str(AlignedString(self.DNSName).encode('latin-1'))
        except:
            return struct.pack('<HHHH', \
                               # UPN Len \
                               len(self.UPNName), \
...
class PacServerChecksum(PacInfoStructure):
    ...
    def __init__(self, pac, index=0):
        ...
        if self.Type != 6:
            raise ValueError(
                'PAC Information Structure is not Server Checksum')
        self.SigType = struct.unpack('<l', self.Data[:4].encode('latin-1'))[0]
        self.SigVal = self.Data[4:]

    def __str__(self):
        return '%s (%i) Size: %i  Sig Type: %i  SigVal: %s' % (self.PrettyName, self.Type, len(self.encode()), self.SigType, self.SigVal.encode('latin-1'))

    def encode(self):
        # return self.Data
        return struct.pack('<l', self.SigType) + self.SigVal.encode('latin-1')
...
class PacKdcChecksum(PacInfoStructure):
    ...
    def __init__(self, pac, index=0):
        ...
        if self.Type != 7:
            raise ValueError(
                'PAC Information Structure is not KDC (Privsvr) Checksum')
        self.SigType = struct.unpack('<l', self.Data[:4].encode('latin-1'))[0]
        self.SigVal = self.Data[4:]
...
class PAC(object):
    ...
    def load(self, pac):
        numentries, self.Version = struct.unpack(
            '<II', pac[:8].encode('latin-1'))
        ...
        for i in range(numentries):
            ...
            typeval = struct.unpack(
                '<L', pac[offset:offset+4].encode('latin-1'))[0]
    ...
    def encode(self):
        ...
        pacstructs = [self.PacLoginInfo, self.PacClientInfo,
                      self.PacUpnDnsInfo, self.PacServerChecksum, self.PacKdcChecksum]

        for ps in pacstructs:
            pse = ps.encode()
            psealigned = AlignedString(pse, 8)

            headers += struct.pack('<IIII', ps.Type, len(pse), offset, 0).decode('latin-1')
            payload += psealigned.decode('latin-1')
        ...
        return e.decode('latin-1') + headers + payload
...
def cmp(s1, s2, comparelen=None, verbose=False):
    ...
    if s1[:comparelen] == s2[:comparelen]:
        ...
        if verbose:
            print('NOT SAME')
            print(s1[:comparelen].encode('latin-1'))
            # print(s1[:comparelen].encode('hex'))
            print()
            print(s2[:comparelen].decode('latin-1'))
